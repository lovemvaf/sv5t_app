...
def pdf_rule_checklist(x: Dict) -> List[str]:
    ...
    if int(x["marx_team_member"]) != 1 and int(x["good_deed_awarded"]) != 1:
        tips.append("Đạo đức: Cần thêm 1 tiêu chí (đội thi Mác–Lênin/HCM hoặc 'người tốt việc tốt' huyện/tỉnh+).")
    ...
    if not academic_plus:
        tips.append("Học tập: Cần thêm 1 tiêu chí học thuật (NCKH/bài báo/kỷ yếu/giải/đội tuyển…).")

    # Thể lực
    if int(x["sv_khoe_provincial_or_higher"]) != 1 and int(x["sport_award_school_or_higher"]) != 1:
        tips.append("Thể lực: Cần đạt 'SV khỏe' (từ tỉnh) hoặc có giải thể thao phong trào cấp trường+.")
    ...
    if int(x["international_exchange"]) != 1 and int(x["integration_competition_award"]) != 1:
        tips.append("Hội nhập: Cần thêm 1 tiêu chí (giao lưu quốc tế hoặc đạt giải thi hội nhập/ngoại ngữ).")
