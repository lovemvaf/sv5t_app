...
def main():
    ...
    # -------------------------
    with tab1:
        st.subheader("1) Nhập hồ sơ → dự đoán → checklist tiêu chí")
        ...
        with st.sidebar:
            ...
            # Fixed: logic reversed. If select "Có" (Yes) -> Has violation -> no_violation = 0.
            # If select "Không" (No) -> No violation -> no_violation = 1.
            vi_pham_input = st.selectbox("Có vi phạm pháp luật/quy chế không?", [0, 1], format_func=lambda v: "Không" if v == 0 else "Có")
            no_violation = 1 if vi_pham_input == 0 else 0
            ...
            good_deed_awarded = st.selectbox(
                "Được biểu dương 'người tốt việc tốt' (huyện/tỉnh/trường+)",
                [0, 1],
                format_func=lambda v: "Có" if v == 1 else "Không",
            )

            st.markdown("### Học tập (chỉ cần 1 tiêu chí học thuật)")
            nckh_good = st.checkbox("NCKH SV loại tốt (Trường/Khoa)")
            thesis_award = st.checkbox("Luận văn/đồ án đạt giải Ba QG/QT")
            ...
            sv_khoe_provincial_or_higher = st.checkbox("Đạt 'SV Khỏe' (tỉnh+) / thể thao TW")
            ...
            # Threshold for language GPA logic logic
            lang_gpa_threshold = 3.2 if grading_scale == 4 else 8.0
            
            foreign_language_gpa = st.number_input(
                f"Điểm tích luỹ ngoại ngữ (nếu không có chứng chỉ, cần >= {lang_gpa_threshold})",
                min_value=0.0,
                max_value=float(grading_scale),
                value=float(lang_gpa_threshold),
                step=0.01,
            )
            ...
            integration_competition_award = st.selectbox(
                "Đạt giải Ba thi hội nhập/ngoại ngữ (trường+)",
                [0, 1],
                format_func=lambda v: "Có" if v == 1 else "Không",
            )
        ...
        # -----------------------------
        # HARD RULES CHECK (VETO LOGIC)
        # -----------------------------
        # -----------------------------
        # HARD RULES CHECK (STRICT VETO LOGIC - QĐ/TWHSV)
        # -----------------------------
        veto_reasons = []
        
        # 1. Đạo đức (Morality)
        if x["ren_luyen"] < 90:
            veto_reasons.append("Đạo đức: Điểm rèn luyện < 90 (yêu cầu >= 90)")
        if x["no_violation"] == 0:
            veto_reasons.append("Đạo đức: Có vi phạm kỷ luật/pháp luật")
        # Strict: Must have 1 of the Plus criteria
        if x["marx_team_member"] == 0 and x["good_deed_awarded"] == 0:
             veto_reasons.append("Đạo đức: Thiếu tiêu chí bổ sung (Đội thi Mác-Lênin/HCM hoặc 'Người tốt việc tốt')")

        # 2. Học tập (Academic)
        gpa_req = 3.4 if (school_level == "daihoc_hocvien" and grading_scale == 4) else \
                  8.5 if (school_level == "daihoc_hocvien" and grading_scale == 10) else \
                  3.2 if (school_level == "caodang" and grading_scale == 4) else 8.0
        if x["gpa"] < gpa_req:
            veto_reasons.append(f"Học tập: GPA {x['gpa']} < {gpa_req} (ngưỡng quy định)")
        
        # Strict: Must have 1 of the Academic Plus criteria
        ...
        if not academic_plus:
            veto_reasons.append("Học tập: Thiếu tiêu chí học thuật bổ sung (NCKH/Giải thưởng/Bài báo...)")
            
        # 3. Thể lực (Physical)
        if x["sv_khoe_provincial_or_higher"] == 0 and x["sport_award_school_or_higher"] == 0:
            veto_reasons.append("Thể lực: Chưa đạt danh hiệu 'SV Khỏe' hoặc giải thể thao theo quy định")

        # 4. Tình nguyện (Volunteering)
        if x["volunteer_days"] < 5:
             veto_reasons.append(f"Tình nguyện: Số ngày {x['volunteer_days']} < 5 ngày")
        if x["volunteer_award_prov_or_district"] == 0:
             veto_reasons.append("Tình nguyện: Thiếu giấy khen/chứng nhận cấp Huyện/Tỉnh/Trường (theo quy định cụ thể)")

        # 5. Hội nhập (Integration)
        # 5.1 Skill course or Award
        if x["skill_course_or_youth_union_award"] == 0:
             veto_reasons.append("Hội nhập: Thiếu khóa kỹ năng hoặc khen thưởng Hội/Đoàn")
        # 5.2 Activity count
        if x["integration_activity_count"] < 1:
             veto_reasons.append("Hội nhập: Chưa tham gia hoạt động hội nhập nào")
        # 5.3 Language
        lang_ok = (x["english_b1_or_equivalent"] == 1) or (x["foreign_language_gpa"] >= lang_gpa_threshold)
        if not lang_ok:
            veto_reasons.append(f"Hội nhập: Chưa đạt chuẩn ngoại ngữ (B1 hoặc điểm >= {lang_gpa_threshold})")
        # 5.4 Plus criteria
        if x["international_exchange"] == 0 and x["integration_competition_award"] == 0:
            veto_reasons.append("Hội nhập: Thiếu tiêu chí bổ sung (Giao lưu QT hoặc Giải thưởng hội nhập)")
        ...
        if veto_reasons:
            pred = 0
            st.warning(f"⚠️ **CẢNH BÁO:** Bạn có điểm hồ sơ (ML Score) là **{prob*100:.1f}%**, TUY NHIÊN bạn chưa đạt danh hiệu do thiếu các điều kiện cứng sau đây:")
        ...
        c1.metric("Xác suất đạt (ML Model)", f"{prob*100:.1f}%")
        ...
        st.markdown("### B) Checklist chi tiết (Luật PDF)")
        checklist_items = pdf_rule_checklist(x)
        if not checklist_items:
             st.success("✅ Bạn đã đáp ứng đủ các tiêu chí cơ bản theo quy định!")
        else:
             for t in checklist_items:
                st.write("⚠️ " + str(t))
        ...
        if pick != "(không chọn)":
            ...
            # Update Hard Constraint fields based on activity metadata
            act_info = acts[acts["activity_id"] == pid].iloc[0]
            
            # Simulation: If activity is Volunteering -> Add days (heuristic +3 days)
            if "tinh_nguyen" in str(act_info["criteria"]):
                 x2["volunteer_days"] += 3
            
            # Simulation: If activity is Integration -> Add count
            if "hoi_nhap" in str(act_info["criteria"]):
                 x2["integration_activity_count"] += 1
            
            # Simulation: If has specific criteria keywords, assume they fix the missing boolean
            # (Simple heuristic for demo)
            if "SV Khỏe" in act_info["name"]:
                 x2["sv_khoe_provincial_or_higher"] = 1
            
            prob2 = _predict_prob(df, bundle, x2)
            
            # Re-check VETO for x2 to see if status improved
            veto2 = []
            if x2["ren_luyen"] < 90: veto2.append("ren_luyen")
            if x2["no_violation"] == 0: veto2.append("no_violation")
            if x2["gpa"] < gpa_req: veto2.append("gpa_req")
            
            # Physical Check
            if x2["sv_khoe_provincial_or_higher"] == 0 and x2["sport_award_school_or_higher"] == 0:
                 veto2.append("physical")
                 
            # Volunteer Check
            if x2["volunteer_days"] < 5: veto2.append("volunteer_days")
            
            # Integration Check
            if x2["integration_activity_count"] < 1: veto2.append("integration_count")
            
            # If Veto persists, prob2 conceptually is 0 for prediction, 
            # but we show Model Score improvement.
            
            delta = prob2 - prob
            st.write(f"Trước: **{prob*100:.1f}%** → Sau: **{prob2*100:.1f}%** (Δ = {delta*100:+.1f}%)")
            
            if veto_reasons and not veto2:
                 st.success("🎉 Hoạt động này giúp bạn khắc phục được các tiêu chí cứng còn thiếu (giả định)!")
            elif veto_reasons and veto2:
                 st.info("ℹ️ Hoạt động này giúp tăng điểm hồ sơ, nhưng bạn vẫn cần bổ sung thêm các tiêu chí cứng khác.")
